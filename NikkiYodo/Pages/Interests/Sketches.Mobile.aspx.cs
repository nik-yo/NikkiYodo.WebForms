...
{
    public partial class SketchesMobile : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                using (MySqlConnection connection = new MySqlConnection(WebConfigHelper.GetDefaultConnectionString()))
                {
                    ...
                    var command = new MySqlCommand("SELECT * FROM sketches", connection);
                ...
                rptSketches.DataSource = dataTable;
                rptSketches.DataBind();
            } 
        }
    }
}